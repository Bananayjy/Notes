...
总结一下，根据条件对表进行查询的时候，什么时候需要回表，什么时候需要在业务层进行数据的过滤而不能光通过存储层完成数据的筛选过滤？
...
```
SELECT B, C FROM t WHERE B = 'xxx' AND C = 'yyy';
```

即查找的B和C字段都可以在二级索引中获取，无需回表，并且其select的字段也都在二级索引中，可以直接获取。

- 场景2：二级索引可以完成筛选，但是需要回表

```
SELECT * FROM t WHERE B = 'xxx' AND C = 'yyy';
```

 `B` 和 `C` 都在二级索引中，且查询条件只涉及 `B` 和 `C`，可以在二级索引中直接完成过滤的。但是由于 `SELECT *` 需要返回所有字段，而二级索引中只包含 `B` 和 `C` 以及主键值，因此 MySQL 仍然需要回表到主键索引中获取完整的行数据

- 场景三：二级索引无法完成筛选，需要回表

```
SELECT * FROM t WHERE B = 'xxx' AND D = 'zzz';
```

由于 `D` 不在索引 `idx_B_C` 中，MySQL 只能利用索引过滤 `B = 'xxx'` 的数据，然而，`D` 列 不在索引中，所以数据库引擎无法在索引扫描时直接对 `D` 列进行过滤（回表，去主键索引中只不过是查询对应记录的全部字段，即select中的*，对于D条件的过滤应该是放在服务层去做的）.需要在服务层完成对数据D的二次筛选
